...
class MenuController extends Controller
{
    ...
    public function show(Request $request, $slug)
    {
        ...
        switch ($menu->type) {
            case 'berita':
                $query = Artikel::with('kategori')->where('status', 1);
                ...
                return view('berita.list', compact('artikels', 'menu', 'search', 'kategoris', 'kategori_id'));
            ...
            case 'kegiatan':
                $query = Kegiatan::with('kategori')->where('status', 1);
                ...
                $kegiatans = $query->latest()->paginate(12)->appends($request->only(['search', 'kategori']));
                $kategoris = Kategori::all();

                return view('kegiatan.list', compact('kegiatans', 'menu', 'search', 'kategoris', 'kategori_id'));
